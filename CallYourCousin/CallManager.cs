...
public class CallManager : MonoBehaviour
{
    ...
    public AudioClip[] rallyVoiceLines;
    public AudioClip[] nearbyVoiceLines;
    public AudioClip[] midPickupVoiceLines;
    public AudioClip[] missedVoiceLines;
    public AudioClip[] annoyedVoiceLines;
    public AudioClip[] confirmVoiceLines;
    ...
    public IEnumerator MakeCall(int waypoint, Action<bool> onCallEnd, bool homeRoute = false)
    {
        ...
        if (response.completedCall)
        {
            ...
            yield return StartCoroutine(PlayVoiceLine(response.message));
    ...
    AudioClip GetVoiceAudioClip(string subtitle)
    {
        AudioClip[] audioClips;
        switch (subtitle)
        {
            case Constants.Subtitles.COUSIN_NEARBY:
                audioClips = nearbyVoiceLines;
                break;

            case Constants.Subtitles.COUSIN_MID_PICKUP:
                audioClips = midPickupVoiceLines;
                break;

            case Constants.Subtitles.COUSIN_MISSED_PICKUP:
                audioClips = missedVoiceLines;
                break;

            case Constants.Subtitles.COUSIN_ANNOYED:
                audioClips = annoyedVoiceLines;
                break;

            case Constants.Subtitles.COUSIN_RALLY:
                audioClips = rallyVoiceLines;
                break;

            case Constants.Subtitles.COUSIN_CONFIRM_PICKUP:
                audioClips = confirmVoiceLines;
                break;
        ...
        int chosenClipIndex = UnityEngine.Random.Range(0, audioClips.Length);
        return audioClips[chosenClipIndex];
    }

    IEnumerator PlayVoiceLine(string subtitle)
    {
        AudioClip audioClip = GetVoiceAudioClip(subtitle);
        if (audioClip == null)
            yield return null;
        ...
        {
            Utils.subtitle.Value = subtitle;
            yield return new WaitForEndOfFrame();
        }

        if (Utils.subtitle.Value == subtitle)
